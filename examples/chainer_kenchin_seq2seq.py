...
#
# Usage:
#   python examples/chainer_kenchin_seq2seq.py -m train -c configs/chainer_kenchin_seq2seq.ini
#   python examples/chainer_kenchin_seq2seq.py -m eval -c configs/chainer_kenchin_seq2seq.ini
#
# Purpose:
...
from networks.kenchin_seq2seq import LSTM_Encoder, LSTM_Decoder, Seq2Seq, forward 
...
def main(args):
  ...
  # optimizer
  # decoder and optimizer for evaluate in train mode
  model = Seq2Seq(vocab_size=vocabulary_size,
  ...
  if args.mode == 'train':
    ...
    for batch in range(train_step):
      ...
      while True: # minibatch process
        ...
        total_loss = forward(enc_words=batch_data['encoder_inputs'],
                             dec_words=batch_data['decoder_inputs'],
                             model=model,
                             ARR=ARR)
        total_loss.backward()
        opt.update()
        ...
        if global_step % loss_freq == 0:
          ...
          # c3. calc loss
          loss = forward(enc_words=batch_data['encoder_inputs'],
                         dec_words=batch_data['decoder_inputs'],
                         model=model,
                         ARR=ARR)
